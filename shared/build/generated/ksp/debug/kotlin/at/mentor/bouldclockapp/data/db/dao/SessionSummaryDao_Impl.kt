...
public class SessionSummaryDao_Impl(
  ...
  public override fun observeAll(): Flow<List<SessionSummaryEntity>> {
    val _sql: String = "SELECT * FROM session_summary ORDER BY startedAt DESC"
    return createFlow(__db, false, arrayOf("session_summary")) { _connection ->
      val _stmt: SQLiteStatement = _connection.prepare(_sql)
      try {
        val _columnIndexOfSessionId: Int = getColumnIndexOrThrow(_stmt, "sessionId")
  ...
  public override suspend fun deleteForSession(sessionId: String) {
    val _sql: String = "DELETE FROM session_summary WHERE sessionId = ?"
